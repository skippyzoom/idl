...
pro set_data_units, context,units, $
  ...
  if context.haskey('data') then begin
     ...
     context.data['units'] = dictionary(d_keys)
     for ik=0,context.data.array.count()-1 do begin
        data_oom = fix(alog10(1./context.data.scale[d_keys[ik]]))
        data_prefix = units.prefixes.where(data_oom)
        case 1 of
           strcmp(d_keys[ik],'den',3): begin
              if keyword_set(absolute) then begin
                 if data_oom eq 0 then context.data.units[d_keys[ik]] = $
                    units.bases['abs_den'] $
                 else context.data.units[d_keys[ik]] = $
                    strcompress(string(context.data.scale[d_keys[ik]]), $
                                /remove_all)+ $
                    "$\times$"+units.bases['abs_den']
              endif $
              else begin
                 case data_oom of
                    0: context.data.units[d_keys[ik]] = $
                       data_prefix.remove()+units.bases['rel_den']
                    -2: context.data.units[d_keys[ik]] = "%"
                    else: context.data.units[d_keys[ik]] = $
                       "$\times$"+ $
                       strcompress(string(context.data.scale[d_keys[ik]]), $
                                   /remove_all)
           ...
           strcmp(d_keys[ik],'phi',3): $
              context.data.units[d_keys[ik]] = data_prefix.remove()+units.bases['phi']
           strcmp(d_keys[ik],'e',1,/fold_case): $
              context.data.units[d_keys[ik]] = data_prefix.remove()+units.bases['E']
        endcase
